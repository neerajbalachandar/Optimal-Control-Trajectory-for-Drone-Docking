...
def run():
    ...
    # --- WIND CONFIGURATION ---
    # Nominal wind vector (e.g., blowing slightly sideways and down)
    WIND_NOMINAL = np.array([0.5, -0.3, -0.1]) 
    WIND_GUST_AMP = 0.3 # Magnitude of random noise
    ...
    for i in range(len(traj)):
        ...
        # --------------------------------------------------------------------
        # APPLY ENVIRONMENTAL DISTURBANCE (WIND)
        # --------------------------------------------------------------------
        # 1. Generate stochastic wind (Mean + Random Gust)
        gust = np.random.uniform(-WIND_GUST_AMP, WIND_GUST_AMP, 3)
        current_wind_force = WIND_NOMINAL + gust
        
        # 2. Apply external force to the Chaser (Drone 0)
        # We apply this in World Frame to the drone's center of mass
        p_chaser = obs[0][0:3]
        p.applyExternalForce(
            objectUniqueId=env.DRONE_IDS[0],
            linkIndex=-1, # -1 = Base Link / CoM
        ...
        # 3. Visualize Wind Vector (Yellow Line pushing the drone)
        # Scale line for visibility
        p.addUserDebugLine(
            p_chaser, 
            p_chaser + current_wind_force, 
            lineColorRGB=[1, 1, 0], 
            lineWidth=2, 
            lifeTime=0.1, # Disappear quickly to animate
            physicsClientId=PYB_CLIENT
        )
        # --------------------------------------------------------------------
