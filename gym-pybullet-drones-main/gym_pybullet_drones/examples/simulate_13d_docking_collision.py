...
from scp_13d_pose_dcol_planner import plan_scp_13d_docking
...
def run():
    ...
    traj = plan_scp_13d_docking(
        x0,
        p_target,
        q_target,
        box_chaser=np.array([0.15,0.15,0.05]),
        box_target=np.array([0.15,0.15,0.05])
    )
    ...
    hull = p.createMultiBody(0,-1,hull_vis,p_start)
