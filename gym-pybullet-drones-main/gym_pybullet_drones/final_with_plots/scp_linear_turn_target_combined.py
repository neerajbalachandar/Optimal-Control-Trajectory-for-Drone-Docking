...
DT_PLAN = 0.1           
DURATION_SEC = 25.0     
TURN_TIME = 5.0         # Target turns at 4 seconds

# --- DOCKING CONFIG ---
DOCKING_AXIS = np.array([0.0, 0.0, -1.0]) # Target expects approach from top
DOCK_OFFSET  = np.array([0.0, 0.0, 0.4])  # Phase 0: Hover 40cm ABOVE target
CONE_ANGLE   = 30    
SAFETY_R     = 0.1   
ALPHA_LIMIT  = 1.05  
...
class SuddenTurnTarget:
    def get_state(self, t):
        s = np.zeros(6)
        speed = 0.05 # Increased speed slightly to test lag handling
...
class TargetEKF:
    ...
    def predict_future(self, steps, dt_plan):
        ...
        # LAG FIX: Removed aggressive damping. 
        # Damping causes the planner to aim behind the target.
        damping = 1.0 
    ...
    def _solve_scp(self, start, preds):
        ...
        for _ in range(2):
            ...
            for k in range(self.N-1):
                ...
                # We trust 'preds' to be the EXACT target location (offset logic is external)
                target_k = preds[k+1, 0:3] 
                
                # Increasing weight over horizon to encourage "catching up"
                w = 1.0 + (0.1 * k) 
                cost += w * 40 * cp.sum_squares(x[0:3, k+1] - target_k)
            ...
            for k in range(1, self.N):
                ...
                # Cone Constraint (Relative to the provided prediction)
                p_rel = x[0:3,k] - preds[k, 0:3] 
                dist_long = -n_app @ p_rel
                con += [dist_long >= -0.2] # Relaxed slightly for robustness
                con += [cp.norm(p_rel) * cos_theta <= dist_long + 0.3 + slack_cone[k]] # Relaxed
                
                con += [cp.norm(x[:,k] - x_ref[:,k]) <= 0.8]

            cost += cp.sum(slack_obs)*1e5 + cp.sum(slack_cone)*1e4
...
def run():
    ...
    # --- PHASE LOGIC ---
    docking_phase = 0 # 0: Offset Approach, 1: Terminal Docking
    frozen = False
    freeze_pos_c = None
    freeze_pos_t = None
    
    print("[SIM] Running Sudden Turn with 2-Phase Docking...")
    START = time.time()
    ...
    for i in range(int(DURATION_SEC * CTRL_FREQ)):
        ...
        # --- PHASE TRANSITION ---
        target_offset_pt = p_target + DOCK_OFFSET
        
        if docking_phase == 0:
            # Visualize the Offset Point (The "Ghost" target)
            # p.addUserDebugLine(p_target, target_offset_pt, [1,1,1], 1, lifeTime=0.1, physicsClientId=PYB)
            
            dist_to_offset = np.linalg.norm(p_chaser - target_offset_pt)
            if dist_to_offset < 0.2:
                docking_phase = 1
                print(f"[PHASE] {sim_t:.2f}s | Switched to Terminal Phase (Dive In!)")

        # --- DOCKING COMPLETION ---
        dist_real = np.linalg.norm(p_chaser - p_target)
        if docking_phase == 1 and dist_real < 0.15:
            plot_performance(history)
            
            frozen = True
            freeze_pos_c = p_chaser
            freeze_pos_t = p_target
            print(f"[COMPLETE] {sim_t:.2f}s | DOCKED! FREEZING.")
            continue

        # 4. CONTROL LOGIC
        if state == STATE_TRACKING:
            if alpha_obs < ALPHA_LIMIT:
                print(f"[ALERT] Obstacle Risk! Backing Off.")
                ...
                if sim_t - last_plan > PLAN_INTERVAL:
                    ...
                    # --- CRITICAL: MODIFY TARGET BASED ON PHASE ---
                    if docking_phase == 0:
                        # In phase 0, we lie to the planner: "The target is 40cm higher"
                        # This forces the drone to fly ABOVE the target, handling lag/turns easily.
                        preds[:, 0:3] += DOCK_OFFSET
                    
                    # In phase 1, we send raw preds (Dive into the target)
                    ...
                    # Draw Planner Goal
                    for j in range(HORIZON-1):
                        color = [0,1,0] if docking_phase == 1 else [1,1,0]
                        # p.addUserDebugLine(preds[j,0:3], preds[j+1,0:3], color, 1, physicsClientId=PYB)

                # --- TRACKING ---
                if curr_traj is not None:
                    # Look ahead 2 steps for aggressive response
                    idx = min(2, curr_traj.shape[1]-1) 
            ...
            res = planner.get()
            if res is not None:
                if np.linalg.norm(res[0:3,0] - p_chaser) < 0.5: # Loose re-entry
                    curr_traj = res
        ...
        # Captures the fresh, nonlinear u* you just calculated
