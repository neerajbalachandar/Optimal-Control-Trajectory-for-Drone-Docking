...
# ======================================================================
# 2. TARGET EKF (9D: Pos, Vel, Acc) [PAPER SECTION III-B]
# ======================================================================
class TargetEKF:
...
# ======================================================================
# 4. ASYNC PLANNER (FULLY PAPER ALIGNED)
# ======================================================================
class AsyncPlanner(threading.Thread):
    ...
    def run(self):
        print("[Planner] Thread Started. Solver: OSQP/SCS")
        while True:
            data = None
            with self.lock:
            ...
            if data:
                try:
                    traj_x = self._solve_scp(data[0], data[1], data[2])
                    if traj_x is not None:
                        with self.lock: self.res = traj_x
    ...
    def _solve_scp(self, start, preds, phase):
        ...
        if self.prev_sol is None:
            ...
            for k in range(self.N):
                ...
                # Curved guess to avoid dead-center singularity
                x_ref[1,k] += 0.5 * np.sin(np.pi*al) 
        else:
            x_ref = np.zeros((6, self.N))
        ...
        # 2. PAPER CONSTANTS: Eq. (20), Eq. (9), Eq. (10)
        # Relaxed Q to prevent aggressive jerks, standard R
        Q_mat = np.diag([1, 1, 5, 0.1,0.1,0.1])  
        R_mat = np.diag([0.1, 0.1, 0.1])        
        
        V_MAX = 5.0                             
        U_MAX = 15.0  # Must be > 9.81 to allow hovering           
        Z_MIN = 0.1   
        
        # Target safety radius (0.0 during dive)
        R_MIN_TARGET = 0.0 if phase == 1 else 0.3 
        ...
        for _ in range(2):
            ...
            # Terminal Relative Velocity Constraint (v_c = v_t)
            ...
            for k in range(1, self.N):
                # Linearized Signed-Distance: Obstacle Avoidance (Eq. 16)
                vec = x_ref[0:3,k] - self.p_obs
                n = vec / (np.linalg.norm(vec)+1e-4)
                con += [n @ (x[0:3,k] - self.p_obs) >= self.r_obs - slack_obs[k]]
                
                # Linearized Cone Constraint (Eq. 15)
                p_rel = x[0:3,k] - preds[k, 0:3] 
                dist_long = -n_app @ p_rel
                con += [dist_long >= 0]
                con += [cp.norm(p_rel) * cos_theta <= dist_long + slack_cone[k]]
                
                # Trust Region: Restored to strict 0.5m to prevent model divergence
                con += [cp.norm(x[:,k] - x_ref[:,k]) <= 0.5] 

            # L1 Slack Penalties (Eq. 20)
            # Tuned to prevent numerical explosion while ensuring strict constraint satisfaction
            cost += cp.sum(slack_obs)*1000 + cp.sum(slack_cone)*1000 + cp.sum(slack_target)*10
...
def run():
    ...
    # Passing slightly inflated R_OBS to prevent FSM override conflicts
    planner = AsyncPlanner(HORIZON, DT_PLAN, P_OBS, R_OBS + SAFETY_R + 0.45, CONE_ANGLE, DOCKING_AXIS)
    planner.start()
