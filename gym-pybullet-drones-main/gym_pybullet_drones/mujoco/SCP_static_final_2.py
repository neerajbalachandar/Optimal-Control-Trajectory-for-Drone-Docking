...
class DroneNMPC:
    def __init__(self, dt=0.1, N=25):
        ...
        self.alpha_min = 1.05  # DCOL multiplier (strictly >= 1.0)
        
        # --- Paper's Anisotropic Cost Matrices ---
        self.Q_pos = np.diag([2.0, 2.0, 5.0])
    ...
    def solve(self, x_true, p_target, phase, is_first_step=False):
        ...
        for it in range(self.MAX_ITERS):
            ...
            offset = np.array([0.0, 0.0, 0.4]) if phase == 0 else np.array([0.0, 0.0, 0.2])
            
            
            # cost += cp.sum_squares(X[-1, 0:3] - (p_target + offset)) * 1000.0
            cost += cp.quad_form(X[-1, 0:3] - (p_target + offset), self.Q_pos) * 1000.0
            ...
            for k in range(self.N-1):
                ...
                if phase == 1:
                    ...
                    con += [n_tar @ p_rel >= (self.r_dock) * self.alpha_min - slack_tar[k]]
