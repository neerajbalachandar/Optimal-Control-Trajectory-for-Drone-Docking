...
class ProjectileTarget:
    def get_state(self, t):
        ...
        # Initial Conditions (Perfectly matched to Linear and Static)
        x_0 = 0.5
        z_0 = 1.0  # <--- Changed from 2.0 to 1.0
        ...
        g   = 0.01  # Reduced from 0.05 to make it "super floaty" 
...
def run():
    ...
    try:
            for i in range(int(DURATION_SEC * CTRL_FREQ)):
                ...
                if docking_phase == 1 and np.linalg.norm(p_chaser - p_target) < 0.25:
                    frozen = True
